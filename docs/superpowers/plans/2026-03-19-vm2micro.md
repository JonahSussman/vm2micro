...
│   │   ├── guestfs.py              # GuestFSBackend (guestmount wrapper)
...
async def test_connect_detects_image_extension() -> None:
    mgr = ConnectionManager()
    # Should detect as image path (won't actually mount, but should select GuestFS)
    with pytest.raises(FileNotFoundError):
...
class ConnectionManager:
    ...
    async def connect(
        ...
        if target.startswith("ssh://"):
            ...
            # Disk image — GuestFS backend
            path = Path(target)
            if not path.exists():
                raise FileNotFoundError(f"Disk image not found: {target}")
            from vm2micro.virtualfs.guestfs import GuestFSBackend
            guestfs_backend = GuestFSBackend()
            await guestfs_backend.mount(target)
    ...
    async def disconnect(self) -> None:
        ...
        if hasattr(self._fs, "unmount"):
            await self._fs.unmount()  # type: ignore[attr-defined]
...
## Task 11: GuestFSBackend

**Files:**
- Create: `src/vm2micro/virtualfs/guestfs.py`, `tests/test_virtualfs_guestfs.py`
- Depends on: Task 3
...
"""Tests for GuestFSBackend. Most tests use mock subprocess to avoid requiring guestmount."""
...
async def test_mount_calls_guestmount() -> None:
    backend = GuestFSBackend()
    with patch("vm2micro.virtualfs.guestfs.asyncio") as mock_asyncio:
        mock_proc = MagicMock()
        mock_proc.returncode = 0
        mock_proc.communicate = AsyncMock(return_value=(b"", b""))
        mock_asyncio.create_subprocess_exec = AsyncMock(return_value=mock_proc)

        with patch("vm2micro.virtualfs.guestfs.Path.exists", return_value=True):
            with patch("vm2micro.virtualfs.guestfs.tempfile") as mock_tempfile:
                mock_tempfile.mkdtemp.return_value = "/tmp/vm2micro-test"
                await backend.mount("/path/to/vm.qcow2")

        # Verify guestmount was called with --ro
        call_args = mock_asyncio.create_subprocess_exec.call_args
        args = call_args[0] if call_args[0] else call_args[1].get("args", [])
        assert "guestmount" in str(args)
        assert "--ro" in str(args)


async def test_mount_nonexistent_image() -> None:
    backend = GuestFSBackend()
    with pytest.raises(FileNotFoundError):
        await backend.mount("/nonexistent/image.qcow2")


async def test_unmount_cleans_up() -> None:
    backend = GuestFSBackend()
    backend._mount_point = "/tmp/fake-mount"
    backend._local = MagicMock()
    ...
    with patch("vm2micro.virtualfs.guestfs.asyncio") as mock_asyncio:
        ...
        with patch("vm2micro.virtualfs.guestfs.shutil") as mock_shutil:
            await backend.unmount()

        assert backend._mount_point is None
```
...
```python
# src/vm2micro/virtualfs/guestfs.py
"""GuestFSBackend: mount disk images via guestmount, delegate reads to LocalPathBackend."""
...
class GuestFSBackend:
    """Read-only access to disk images via guestmount."""

    def __init__(self) -> None:
        self._mount_point: str | None = None
        self._local: LocalPathBackend | None = None

    async def mount(self, image_path: str) -> None:
        """Mount a disk image read-only via guestmount."""
        if not Path(image_path).exists():
            raise FileNotFoundError(f"Image not found: {image_path}")

        mount_dir = tempfile.mkdtemp(prefix="vm2micro-")

        proc = await asyncio.create_subprocess_exec(
            "guestmount", "--ro", "-a", image_path, "-i", mount_dir,
        ...
        if proc.returncode != 0:
            ...
            raise RuntimeError(
                f"guestmount failed (exit {proc.returncode}): {stderr.decode()}"
            )

        self._mount_point = mount_dir
        self._local = LocalPathBackend(mount_dir)
    ...
    async def unmount(self) -> None:
        ...
        proc = await asyncio.create_subprocess_exec(
            "guestunmount", self._mount_point,
            stdout=asyncio.subprocess.PIPE,
            stderr=asyncio.subprocess.PIPE,
        )
    ...
    def _require_mounted(self) -> LocalPathBackend:
        if self._local is None:
            raise RuntimeError("Not mounted — call mount() first")
        return self._local

    async def read_file(self, path: str, max_lines: int | None = None) -> str:
        return await self._require_mounted().read_file(path, max_lines=max_lines)

    async def list_dir(self, path: str) -> list[DirEntry]:
        return await self._require_mounted().list_dir(path)

    async def exists(self, path: str) -> bool:
        return await self._require_mounted().exists(path)

    async def glob(self, pattern: str) -> list[str]:
        return await self._require_mounted().glob(pattern)

    async def stat(self, path: str) -> FileStat:
        return await self._require_mounted().stat(path)

    async def read_link(self, path: str) -> str:
        return await self._require_mounted().read_link(path)
```
...
```bash
uv run mypy src/vm2micro/virtualfs/guestfs.py
```

- [ ] **Step 6: Commit**

```bash
git add src/vm2micro/virtualfs/guestfs.py tests/test_virtualfs_guestfs.py
git commit -m "feat: GuestFSBackend — disk image analysis via guestmount"
```
...
class CloudDiskBackend:
    ...
    def _not_implemented(self) -> None:
        raise NotImplementedError(
            ...
            "For now, use one of:\n"
            "  - Download/export the disk image and use a local path or guestmount\n"
...
| 11 | GuestFSBackend | `virtualfs/guestfs.py` |
